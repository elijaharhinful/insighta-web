...
export default function LoginPage() {
  ...
  async function handleGitHubLogin() {
    setLoading(true);
    try {
      const res = await fetch("/api/proxy/auth/github");
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center justify-center overflow-hidden rounded-2xl border border-zinc-200 bg-white/50 p-8 shadow-xl backdrop-blur-xl dark:border-zinc-800 dark:bg-black/50">
        ...
        <div className="w-full space-y-4">
          <Button
            ...
            disabled={loading}
            className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium h-11 px-8 w-full bg-[#24292F] text-white hover:bg-[#24292F]/90 dark:bg-white dark:text-black dark:hover:bg-zinc-200"
          >
